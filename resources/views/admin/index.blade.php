...
@section('page-title', 'Kelola Surat Klinis')
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/50">
            <div>
                ...
                <p class="text-xs text-slate-500 mt-1 font-medium italic">Total: {{ $files->count() }} dokumen terunggah</p>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500 font-bold">
                        ...
                        <th class="px-6 py-4 border-b border-slate-100">Unit Kerja (KSM/Spesialis)</th>
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-slate-700 group-hover:text-blue-800 transition-colors">{{ $f->title }}</span>
                                <span class="text-[10px] text-slate-400 font-medium">Diunggah: {{ $f->created_at->format('d M Y') }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
